...
public class AggregationConfigServiceImpl implements AggregationConfigService {
	...
	public List<ValueObject> getAggregationTypes() {
		return Arrays.asList(AggregationType.values()).stream().map(v -> {
			...
			object.setName(v.getDisplayName());
			return object;
		}).collect(Collectors.toList());
	}
	...
	public List<?> getAllIndicators() {
		return indicatorRepository.getNumberIndicators().stream().map(v -> {
			...
			indicatorModel.setIndicatorId(Integer.valueOf(String.valueOf(v.getIndicatorDataMap().get("indicatorNid"))));
	...
	public String saveIndiactor(IndicatorConfigModel dataModel) {
		try {
			Map<Integer, Question> questions = questionRepository
					.findAllByFormIdOrderByQuestionOrderAsc(Integer.parseInt(dataModel.getFormId())).stream()
			...
			Map<Integer, Map<String, Object>> indicatorsMap = indicatorRepository.getNumberIndicators().stream()
					.collect(
							Collectors.toMap(k -> Integer.valueOf(String.valueOf(k.getIndicatorDataMap().get("indicatorNid"))),
									v -> v.getIndicatorDataMap()));

			Indicator lastIndicatorNid = indicatorRepository.findTopByOrderByIdDesc();

			Indicator indicator = new Indicator();
			indicator.setId(dataModel.getId());
	...
	private Map<String, Object> setIndicatorsValue(IndicatorConfigModel dataModel, Map<Integer, Question> question,
		...
		String indicatorNid = dataModel.getIndicatorNid()!=null?dataModel.getIndicatorNid():(lastIndicatorNid != null)
				? String.valueOf(Integer.parseInt(String.valueOf(lastIndicatorNid.getIndicatorDataMap().get("indicatorNid"))) + 1)
				: "1";

		indicatorMap.put("indicatorNid", indicatorNid);
		...
		indicatorMap.put("parentType",
				dataModel.getParentColumnName() != null
						...
						: ((dataModel.getAggregationType().equals("percent")
								|| dataModel.getAggregationType().equals("avg") || dataModel.getAggregationType().equals("rate")
								|| dataModel.getAggregationType().equals("per 1000") || dataModel.getAggregationType().equals("per 10,000")
								|| dataModel.getAggregationType().equals("per lakh")) ? "indicator"
		...
		indicatorMap.put("numerator", dataModel.getParentColumnName() != null
				? (dataModel.getParentColumnName().contains("BEGINREPEAT")
						? (dataModel.getParentColumnName() + "."
								+ question.get(Integer.parseInt(dataModel.getQuestionId())).getColumnName())
						: (dataModel.getQuestionId() != null ? question.get(Integer.parseInt(dataModel.getQuestionId())).getColumnName()
								: null))
//				: (dataModel.getAggregationType().equals("percent") || dataModel.getAggregationType().equals("avg"))
				: dataModel.getNumerator() != null
		...
		indicatorMap.put("highIsGood", dataModel.getHighIsGood());
...
//			for (ConditionModel conditionModel : conditions) {
				...
				if (question.get(conditions.get(j).getQuestion()).getParentColumnName().contains("BEGINREPEAT")) {
					if(i==0) {
						aggRule += "and$" + conditions.get(j).getOperator() + "(data."
								+ question.get(conditions.get(j).getQuestion()).getParentColumnName() + "."
								+ question.get(conditions.get(j).getQuestion()).getColumnName() + ":"
								+ conditions.get(j).getValue().intValue() + ");";
					} else {
					aggRule += conditions.get(j).getAssociation().toLowerCase()+ "$" + conditions.get(j).getOperator() + "(data."
							+ question.get(conditions.get(j).getQuestion()).getParentColumnName() + "."
							+ question.get(conditions.get(j).getQuestion()).getColumnName() + ":"
							+ conditions.get(j).getValue().intValue() + ");";
					}
					...
					aggRule += conditions.get(j-1).getAssociation().toLowerCase()+ "$" + conditions.get(j).getOperator() + "(data."
							+ question.get(conditions.get(j).getQuestion()).getColumnName() + ":"
							+ conditions.get(j).getValue().intValue() + ");";
					}
				}
	...
	//Returns list of indicators from primary source to display in Indicator view table
	@Override
	public List<?> getIndicatorsForView() {
		return indicatorRepository.getIndicatorsBySourceIsNull();
	}
	...
	// Creates an excel file with all indicators and returns the filepath.
	@Override
	public String excelDownloadIndicators() {
		...
		try {
			...
			Map<String, List<IndicatorMapModel>> indicatorsMap = indicatorRepository.getIndicatorsBySourceIsNull().stream().map(v -> {
				...
				System.out.println(v.getIndicatorDataMap().get("aggregationType"));
	...
	@Override
	public List<?> getIndicatorsBySector(String formId, String sector, String subsector) {
		// TODO Auto-generated method stub
		return indicatorRepository.getByFormIdSectorSubsector(formId, sector, subsector);
	}
}

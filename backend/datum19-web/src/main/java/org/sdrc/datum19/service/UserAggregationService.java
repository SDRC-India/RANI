 ...
 * description : to aggregate indicators against each users. this will be computed from area wise aggregated data and user-area mapping.
 */
@Service
public class UserAggregationService {
	...
	public void aggregateUserWise(String periodicity,Integer timePeriod) {
		...
		//getting user data by aggregating area wise data
		...
		//stroring the user data 
		userDataValueRepository.saveAll(userDataList);
		try {
			//aggregating the average and percentage indicators
	...
	//to compute percentage indicators from number indicators
	...
	//to cpmpute the avaerage indicators
	...
	private Aggregation getUserData(Integer timePeriodId){
		//joining with data value collection by passing assigned area Ids of the user to get aggregated sum of data for those area
		LookupOperation lookupOperation=Aggregation.lookup("dataValue", "mappedAreaIds", "datumId", "data");
		//unwinding the output field containing values for multiple area
		...
		//grouping by userName and indicator to get aggregated sum of datavalue

...
export class TableComponent implements OnInit, OnChanges {
  ...
  @Output() onresetPasswordClicked: EventEmitter<any> = new EventEmitter<any>();
  @Output() onEditUserClicked: EventEmitter<any> = new EventEmitter<any>();
  @Output() onEnableButtonClicked: EventEmitter<any> = new EventEmitter<any>();
  @Output() onDisableButtonClicked: EventEmitter<any> = new EventEmitter<any>();
  ...
  ngOnChanges() { 
    ...
    if (this.columnData){
      ...
      for (let index = 0; index < this.columnValues.length; index++) {            
        if(this.columnValues[index] == 'siNo')  {
          this.columnValues[index] = 'Sl. No.';
        }
        if(this.columnValues[index] == 'name'){
          this.columnValues[index] = 'Full Name';
        }
        if(this.columnValues[index] == 'userName')  {
          this.columnValues[index] = 'Username';
        }
        if(this.columnValues[index] == 'roleNames'){
          this.columnValues[index] = 'User Role';          
        }
        if(this.columnValues[index] == 'areaName'){
          this.columnValues[index] = 'Area';          
        }
      }
    }
  }
  ...
  public resetPasswordByClicked(tableRow){
    let emittedData: any = {tableRow: tableRow};
    this.onresetPasswordClicked.emit(emittedData);    
  }
  public editUserByClicked(tableRow){
    let emittedData: any = tableRow;
    this.onEditUserClicked.emit(emittedData);    
  }
  public enableUserByClicked(tableRow){
    let emittedData: any = tableRow;
    this.onEnableButtonClicked.emit(emittedData);    
  }
  public disableUserByClicked(tableRow){
    let emittedData: any = tableRow;
    this.onDisableButtonClicked.emit(emittedData);    
  }
}
